...
public class SortItemsByGroupsRespectingDependenciceTopoSortLeetcode1203 {
	...
	public int[] sortItems(int n, int m, int[] group, List<List<Integer>> beforeItems) {
		...
		// Each item belongs to a group. If an item doesn't have a group it forms it's
		// own isolated group.
		for (int i = 0; i < group.length; i++) {
			if (group[i] == -1)
				group[i] = m++;
		...
		// This Map holds relative order of items in the same group computed in the loop
		// below.
